...
비교 기반 정렬에서는 최소한 
log2(𝑁!)번의 비교가 필요합니다.
이 값은 Stirling's 근사를 사용하면 대략𝑁log𝑁에 비례합니다.
따라서, 어떤 비교 기반 정렬 알고리즘이라도 𝑂(𝑁log⁡𝑁)보다 빠를 수는 없습니다.
...
1. STL의 메모리 관리
동적 메모리 할당 및 관리 (std::allocator)
컨테이너 내부에서의 메모리 최적화
메모리 릭 방지

1. STL과 멀티스레딩
멀티스레딩 환경에서 STL 사용 시 주의점
동기화 기법 (예: std::mutex, std::lock_guard 등)
스레드 안전성을 보장하는 컨테이너 및 알고리즘

1. STL의 예외 처리
STL에서의 예외 처리와 안전성
std::out_of_range, std::bad_alloc과 같은 예외
예외가 발생할 수 있는 상황에서의 안전한 코드 작성법


DB쿼리 튜닝(OR은 index를 안타므로 union으로 해결을 포함시키자) 실제경험 (목업데이터 가득 채우고 해보자)

트래픽 과부화 해보자(스트레스 테스트, 로드 테스트로 병목해결 들)

게이트웨이, sso, 리버스프록시 관계

패브릭 네트워크(이더넷 패브릭) _ https://www.bloter.net/news/articleView.html?idxno=12756
...
인프라 보안: ACL, 디도스
서비스 보안: API 공격 뚫리나 등
개인정보정책보안: API에 개인정보 포함?, 디비 암호화 되어있나

부하테스트
네트워크 DB 샵인키, 캐시
DB에 패킷 이상한거 집어넣어보기
천만개 SELECT 디지게 보내서 자빠뜨리기
풀, 특정 조건 인덱스 처리로 디비 조회
xplan(정확하지 않음)을 쿼리 앞에 붙여 쿼리가 실행된 시간을 체크할 수 있다.(FULL 스캔 경고)
